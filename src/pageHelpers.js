...
// mapPool:    string[]  — all available maps (original pool)
// bansNeeded: number    — total bans before the phase ends (pool.length - bestOf)
// p1, p2:     string    — the two players; p1 always bans first
// timeoutMs:  number
...
function waitForMapBans(page, p1, p2, mapPool, bansNeeded, timeoutMs, sendMsg, watchFn) {
  return new Promise((resolve) => {
    ...
    // Alternating turn order: p1, p2, p1, p2, … for bansNeeded total slots
    const banOrder  = Array.from({ length: bansNeeded }, (_, i) => i % 2 === 0 ? p1l : p2l);
    ...
    const stop = watchFn(page, (username, message) => {
      ...
      if (!turn) return;  // all bans already recorded

      // Reject out-of-turn bans
      if (uLow !== turn) {
        const turnName = turn === p1l ? p1 : p2;
        sendMsg(`❌ It's ${turnName}'s turn to ban.`).catch(() => {});
        return;
      }
      ...
      if (!matched) {
        ...
        sendMsg(`❌ "${message.slice(5).trim()}" not found. Available: ${avail.join(', ')}`).catch(() => {});
        return;
      }
      if (bannedLow.has(matched.toLowerCase())) {
        const avail = mapPool.filter(m => !bannedLow.has(m.toLowerCase()));
        sendMsg(`❌ ${matched} is already banned. Available: ${avail.join(', ')}`).catch(() => {});
        return;
      }

      bansDone.push({ player: username, map: matched });
      bannedLow.add(matched.toLowerCase());
      sendMsg(`🚫 ${username} bans ${matched}.`).catch(() => {});

      if (bansDone.length >= bansNeeded) {
        clearTimeout(deadline);
        stop();
        finish(false);
      } else {
        const nextTurn = currentTurn();
        const nextName = nextTurn === p1l ? p1 : p2;
        sendMsg(`🗺️ ${nextName}'s turn to ban.`).catch(() => {});
      }
    });
    ...
    const deadline = setTimeout(() => {
      ...
      while (bansDone.length < bansNeeded) {
        ...
        const turn    = currentTurn();
        const name    = turn === p1l ? p1 : p2;
        bansDone.push({ player: name, map: avail[0], auto: true });
        bannedLow.add(avail[0].toLowerCase());
      }
      finish(true);
    }, timeoutMs);
  });
